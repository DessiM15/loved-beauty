...
import { categoryPanels, noir } from "@/content/site";
          ...
          <li key={p.handle} className="group relative border-b border-line last:border-b-0 sm:border-r sm:[&:nth-child(2n)]:border-r-0 lg:border-b-0 lg:[&:nth-child(2n)]:border-r lg:last:border-r-0">
            <Link href={`/collections/${p.handle}`} className="relative block overflow-hidden" aria-label={`Shop ${p.title}`}>
              <div className="relative aspect-[4/5] sm:aspect-[3/4] lg:aspect-[3/4.4]">
                <ThemedImage
                  light={{ src: p.image, alt: p.alt }}
                  dark={noir.panels[p.handle] ?? { src: p.image, alt: p.alt }}
                  fill
                ...
                />
                <div className="absolute inset-0 transition-opacity duration-700 group-hover:opacity-90" style={{ background: "linear-gradient(180deg, rgba(20,14,16,0) 45%, rgba(20,14,16,0.62) 100%)" }} />
              </div>
              <div className="absolute inset-x-0 bottom-0 p-6 text-[#fbf7f5] md:p-8" data-reveal style={{ "--d": `${i * 90}ms` } as React.CSSProperties}>
                ...
                <p className="mt-3 max-w-[16rem] text-sm text-[#fbf7f5]/80">{p.text}</p>
